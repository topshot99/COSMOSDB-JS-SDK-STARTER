...
async function main(): Promise<void> {
    const { database } = await client.databases.createIfNotExists({ id: process.env.DATABASE});
    const { container } = await database.containers.createIfNotExists({ id: process.env.CONTAINER });
    ...
    setTimeout(() => controller.abort(), parseInt(process.env.QUERY_ABORT_TIME_MS));
    const queryOptions = {  signal ,
    ...
    try {
        const { diagnostics} = await queryIterator.fetchNext({ruCapPerOperation: parseInt(process.env.RU_THRESOLD)});
